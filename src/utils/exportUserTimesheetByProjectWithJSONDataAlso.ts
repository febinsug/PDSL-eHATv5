...
export const exportUserTimesheetByProjectsToExcelAndJSON = (
  projects: Project[],
  user: any,
  ...
  projects.forEach((project) => {
    ...
    allJSNNData[project.name + "==" + project.id] = []
    // Headers
    ...
    // console.log("mergedWeeks", mergedWeeks);
    ...
    Object.keys(mergedWeeks).forEach((weekNumStr) => {
      ...
      weekDays.forEach((day) => {
        if (weekData[day].date) {
          ...
          // allJSNNData[project.name + "==" + project.id].push({
          //   week_number: weekNum,
          //   year: new Date(weekData[day].date).getUTCFullYear(),
          //   date: weekData[day].date,
          //   hours: weekData[day].hours,
          //   status: weekData[day].status || "No Status"
          // })
          allJSNNData[project.name + "==" + project.id].push({
            "project_id": "",
            "user_id": "",
            "weekNumber": weekNum,
            "year": new Date(weekData[day].date).getUTCFullYear(),
            "date": weekData[day].date +" 05:30:00.000",
            "hours": weekData[day].hours,
            "submitted_at": "",
            "note": "",
            "status": weekData[day].status || "No Status",
            "approved_at": "",
            "approved_by": "f8ef4483-853a-4e46-adcf-5b6d69eb352e",
            "rejection_reason": ""
          })
  ...
  // ----- 3️⃣ Save JSON file -----
  const jsonBlob = new Blob([JSON.stringify(allJSNNData, null, 2)], {
    type: "application/json",
  });
  saveAs(
    jsonBlob,
    `${user.full_name || user.username}-timesheet.json`
  );
